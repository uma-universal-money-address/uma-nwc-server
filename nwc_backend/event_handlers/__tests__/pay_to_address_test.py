...
async def test_pay_to_address_success(mock_post: Mock) -> None:
...
async def test_pay_to_address_failure__no_receiver() -> None:
...
async def test_pay_to_address_failure__multiple_receivers() -> None:
    with pytest.raises(InvalidInputException):
        await pay_to_address(
            access_token=token_hex(),
            request=Nip47Request(
...
async def test_pay_to_address_failure__invalid_input() -> None:
...
@patch.object(aiohttp.ClientSession, "post")
async def test_pay_to_address_failure__http_raises(mock_post: Mock) -> None:
    mock_response = AsyncMock()
    mock_response.raise_for_status = Mock(
