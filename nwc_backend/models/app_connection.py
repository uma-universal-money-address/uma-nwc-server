...
import json
from nwc_backend.nwc_command import NWCCommand
...
class AppConnection(ModelBase):
    ...
    def get_required_commands(self) -> list[NWCCommand]:
        command_vals = json.loads(self.required_commands)
        return [NWCCommand(command) for command in command_vals]

    def set_optional_commands(self, commands: list[NWCCommand]) -> None:
        commands_vals = [command.value for command in commands]
        self.optional_commands = json.dumps(commands_vals)

    def get_optional_commands(self) -> list[NWCCommand]:
